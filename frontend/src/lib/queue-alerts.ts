...
export function isAlertSnoozed(control?: AlertControl | null): boolean {
  const until = control?.snooze_until;
...
export function getAcknowledgedSignature(control?: AlertControl | null): string | null {
  return control?.acknowledged_signature ?? null;
}
